...
```mermaid
graph LR
    A[Ganesh OHLC] -->|REST| B[SpreadWatch Core]
...
5. **Publication:** Results pushed to MQ topic `spreadwatch.output` and exposed via REST

## Output Distribution

- **REST APIs** on ports `3022` serve downstream consumers with pagination support

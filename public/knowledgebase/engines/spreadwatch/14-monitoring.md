# SpreadWatch — Monitoring & Alerting

**Version:** 2.8.0 | **Owner:** Analytics | **Last Updated:** 2026-07-25

## Health Monitoring

SpreadWatch health is monitored at 99.5% uptime via Narad heartbeat checks every 5 seconds.
...
```prometheus
# Service health
spreadwatch_up{instance="192.168.190.104:3022"} 1

# Request metrics
spreadwatch_http_requests_total{method="GET", endpoint="/api/v1/health", status="200"}
spreadwatch_http_request_duration_seconds{quantile="0.99"}

# Processing metrics
spreadwatch_messages_processed_total
spreadwatch_processing_duration_seconds{quantile="0.95"}
spreadwatch_cache_hit_ratio
```
...
|-----------|-----|---------|
| Service Overview | `/d/spreadwatch-overview` | Health, latency, error rates |
| API Insights | `/d/spreadwatch-api` | Endpoint usage, rate limiting |
| MQ Consumer | `/d/spreadwatch-mq` | Message processing, lag, DLQ |
| Resource Usage | `/d/spreadwatch-resources` | CPU, memory, disk, network |

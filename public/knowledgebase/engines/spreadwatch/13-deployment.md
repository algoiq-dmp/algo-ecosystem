# SpreadWatch — Deployment Guide

**Version:** 2.8.0 | **Owner:** Analytics | **Last Updated:** 2026-07-25

## Deployment Strategy

SpreadWatch v2.8.0 deploys on **ALGO IQ 4** (`192.168.190.104`) as containerized services managed by Narad deployment orchestrator.
...
```bash
narad deploy spreadwatch --env staging --version 2.8.0 --strategy rolling
```

### 3. Smoke Tests
```bash
parikshak smoke-test --target spreadwatch --env staging
```

### 4. Production Deployment
```bash
narad deploy spreadwatch --env production --version 2.8.0 \
  --strategy blue-green \
...
```bash
narad rollback spreadwatch --env production --version 2.8.0
```

# SpreadWatch — Suraksha Integration

**Version:** 2.8.0 | **Owner:** Analytics | **Last Updated:** 2026-07-25

## Suraksha Security Integration

SpreadWatch integrates with **Suraksha** (Security Layer) for all authentication, authorization, and audit capabilities.
...
```
Client → SpreadWatch API → Suraksha /auth/verify → Validate JWT → Grant/Deny
```

1. Client includes `Authorization: Bearer <token>` header
2. SpreadWatch extracts JWT and calls Suraksha `/auth/verify`
3. Suraksha validates token signature, expiry, and revocation status
4. Response includes user identity and role claims
5. SpreadWatch enforces RBAC based on role claims
...
SpreadWatch retrieves sensitive configuration from Suraksha Vault at startup:
...
| Event | Suraksha Action | SpreadWatch Response |
|-------|----------------|-------------------|

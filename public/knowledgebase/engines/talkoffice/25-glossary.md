...
Key terms used throughout TalkOffice documentation and operations.

### A

**API Gateway:** The public-facing REST interface component (`talkoffice-rms`) that handles authentication, rate limiting, and request routing.
...
**Docker Compose:** Tool used for defining and running multi-container TalkOffice deployments with a single configuration file.
...
**gRPC:** High-performance RPC framework used for internal inter-module communication within TalkOffice.
...
**Webhook:** HTTP callback mechanism where TalkOffice sends POST requests to registered subscriber URLs on data change events.

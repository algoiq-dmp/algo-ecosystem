# TalkOffice — Troubleshooting Guide

**Version:** 4.0.0 | **Owner:** Operations | **Last Updated:** 2026-07-25

## Common Issues & Solutions

This guide covers frequently encountered issues with TalkOffice and their resolution steps.
...
**Diagnosis:**
```bash
docker logs talkoffice-core --tail 50
```

**Common causes:**
1. Database unreachable → Verify `TALKOFFICEDB_URL` and network
2. MQ unreachable → Check RabbitMQ status on port 5672
3. Narad not running → Verify Narad on port 3100
4. Port conflict → Check no other process on ports 3080

**Resolution:**
```bash
# Check dependencies
narad health-check --target talkoffice --include-deps
...
curl http://192.168.190.119:3080/api/v1/metrics | grep latency
```
...
```bash
# Scale up workers
narad scale talkoffice --replicas 4

# Clear and warm cache
curl -X POST http://192.168.190.119:3080/api/v1/admin/cache/warm
```
...
rabbitmqctl list_queues name messages_ready messages_unacknowledged | grep talkoffice
```

**Resolution:**
```bash
# Restart MQ consumer connection
narad restart-component talkoffice --component mq-consumer

# For persistent issues, check network between servers
narad network-test --source 192.168.190.119 --target 192.168.190.118:5672
```
...
**Diagnosis:**
```bash
docker logs talkoffice-core | grep -i migration
```

**Resolution:**
```bash
# Check migration status
docker compose run --rm talkoffice-core ./migrate.sh status

# Repair if stuck
docker compose run --rm talkoffice-core ./migrate.sh repair

# Force rollback and retry
docker compose run --rm talkoffice-core ./migrate.sh undo
...
curl -H "Authorization: Bearer <test_token>" \
  http://192.168.190.119:3080/api/v1/health
```
...
narad reauth talkoffice --service suraksha
docker compose restart talkoffice-api
```
...
narad metrics talkoffice --metric memory_usage --duration 1h
```

**Resolution:**
```bash
# Immediate: restart the service
narad restart talkoffice --graceful

# Long-term: profile with heap dump
# Open ticket with Operations team for investigation
```
...
curl http://192.168.190.119:3080/api/v1/status | jq .last_processed_at
```

**Resolution:**
```bash
# Check upstream data flow
narad pipeline-status --target talkoffice

# Force reprocessing
curl -X POST http://192.168.190.119:3080/api/v1/admin/reprocess \
...
curl -X POST http://192.168.190.119:3080/api/v1/admin/cleanup \
  -d '{"before":"2026-07-18","force":false}'
```

## Escalation Path

1. Check this guide → 2. Check Grafana dashboards → 3. Slack #talkoffice-support → 4. Page Operations on-call → 5. Incident in PagerDuty

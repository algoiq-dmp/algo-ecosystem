# TalkOffice — Testing Strategy

**Version:** 4.0.0 | **Owner:** Operations | **Last Updated:** 2026-07-25

## Test Plan

TalkOffice follows a comprehensive testing strategy validated through **Parikshak** testing engine.
...
- **Location:** `talkoffice/tests/unit/`
- **Run:** Every commit via CI pipeline
- **Time:** < 2 minutes for full suite

### 2. Component Tests
- **Scope:** Each module (`talkoffice-oms, talkoffice-rms, talkoffice-dashboard`) tested in isolation
...
Before any release, TalkOffice must pass Parikshak certification:

```
parikshak certify talkoffice --version 4.0.0 --env staging
...
test:
  stage: test
  script:
    - parikshak run-unit --target talkoffice
    - parikshak run-component --target talkoffice
    - parikshak security-scan --target talkoffice
  only:
    - merge_requests
    - main
```

...
4. **Enrichment:** Cross-referencing with cached historical context from `PostgreSQL`
5. **Publication:** Results pushed to MQ topic `talkoffice.output` and exposed via REST

## Output Distribution

- **REST APIs** on ports `3080` serve downstream consumers with pagination support

# TalkOffice — Suraksha Integration

**Version:** 4.0.0 | **Owner:** Operations | **Last Updated:** 2026-07-25

## Suraksha Security Integration

TalkOffice integrates with **Suraksha** (Security Layer) for all authentication, authorization, and audit capabilities.
...
```
Client → TalkOffice API → Suraksha /auth/verify → Validate JWT → Grant/Deny
```

1. Client includes `Authorization: Bearer <token>` header
2. TalkOffice extracts JWT and calls Suraksha `/auth/verify`
3. Suraksha validates token signature, expiry, and revocation status
4. Response includes user identity and role claims
5. TalkOffice enforces RBAC based on role claims
...
TalkOffice retrieves sensitive configuration from Suraksha Vault at startup:
...
| Event | Suraksha Action | TalkOffice Response |
|-------|----------------|-------------------|

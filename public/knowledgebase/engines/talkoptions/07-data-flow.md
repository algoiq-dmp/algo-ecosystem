# TalkOptions Platform — Data Flow
...
```mermaid
graph LR
    A[Ganesh OHLC] -->|REST| B[TalkOptions Platform Core]
    C[MQ Market Data] -->|Pub/Sub| B
    D[Surya BOD/EOD] -->|REST| B
    E[Lakshmi Live] -->|WebSocket| B
    B --> F[Processing Pipeline]
    F --> G[API Gateway]
    G -->|REST JSON| H[Delta XI, VYUH, SpreadWatch, TalkDelta, AALAP Calls, Simulator, TalkDelta AI]
...
4. **Enrichment:** Cross-referencing with cached historical context from `PostgreSQL, InfluxDB`
5. **Publication:** Results pushed to MQ topic `talkoptions.output` and exposed via REST

## Output Distribution

- **REST APIs** on ports `8081, 8444` serve downstream consumers with pagination support

# TalkOptions Platform — Deployment Guide

**Version:** 4.7.2 | **Owner:** Analytics | **Last Updated:** 2026-07-25

## Deployment Strategy

TalkOptions Platform v4.7.2 deploys on **ALGO IQ 18** (`192.168.190.118`) as containerized services managed by Narad deployment orchestrator.

## Deployment Architecture

```
┌────────────────────────────────────────┐
│         ALGO IQ 18 (192.168.190.118)         │
│  ┌──────────┐ ┌──────────┐ ┌────────┐ │
│  │  Core    │ │   API    │ │Worker  │ │
...
| Production | ALGO IQ 18 | Live trading operations | Senior Engineer + DXCC |
...
```bash
narad deploy talkoptions --env staging --version 4.7.2 --strategy rolling
```

### 3. Smoke Tests
```bash
parikshak smoke-test --target talkoptions --env staging
```

### 4. Production Deployment
```bash
narad deploy talkoptions --env production --version 4.7.2 \
  --strategy blue-green \
...
```bash
narad rollback talkoptions --env production --version 4.7.2
```

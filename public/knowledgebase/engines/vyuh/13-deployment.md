# VYUH — Deployment Guide

**Version:** 3.0.0 | **Owner:** Analytics | **Last Updated:** 2026-07-25

## Deployment Strategy

VYUH v3.0.0 deploys on **ALGO IQ 4** (`192.168.190.104`) as containerized services managed by Narad deployment orchestrator.
...
```bash
narad deploy vyuh --env staging --version 3.0.0 --strategy rolling
```

### 3. Smoke Tests
```bash
parikshak smoke-test --target vyuh --env staging
```

### 4. Production Deployment
```bash
narad deploy vyuh --env production --version 3.0.0 \
  --strategy blue-green \
...
```bash
narad rollback vyuh --env production --version 3.0.0
```

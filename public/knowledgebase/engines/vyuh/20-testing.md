# VYUH — Testing Strategy

**Version:** 3.0.0 | **Owner:** Analytics | **Last Updated:** 2026-07-25

## Test Plan

VYUH follows a comprehensive testing strategy validated through **Parikshak** testing engine.
...
- **Location:** `vyuh/tests/unit/`
- **Run:** Every commit via CI pipeline
- **Time:** < 2 minutes for full suite

### 2. Component Tests
- **Scope:** Each module (`vyuh-engine, vyuh-api`) tested in isolation
...
Before any release, VYUH must pass Parikshak certification:

```
parikshak certify vyuh --version 3.0.0 --env staging
...
test:
  stage: test
  script:
    - parikshak run-unit --target vyuh
    - parikshak run-component --target vyuh
    - parikshak security-scan --target vyuh
  only:
    - merge_requests
    - main
```

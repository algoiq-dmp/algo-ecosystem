...
Key terms used throughout VYUH documentation and operations.

### A

**API Gateway:** The public-facing REST interface component (`vyuh-api`) that handles authentication, rate limiting, and request routing.
...
**Docker Compose:** Tool used for defining and running multi-container VYUH deployments with a single configuration file.
...
**gRPC:** High-performance RPC framework used for internal inter-module communication within VYUH.
...
**Webhook:** HTTP callback mechanism where VYUH sends POST requests to registered subscriber URLs on data change events.

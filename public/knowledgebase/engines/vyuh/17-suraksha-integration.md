# VYUH — Suraksha Integration

**Version:** 3.0.0 | **Owner:** Analytics | **Last Updated:** 2026-07-25

## Suraksha Security Integration

VYUH integrates with **Suraksha** (Security Layer) for all authentication, authorization, and audit capabilities.
...
```
Client → VYUH API → Suraksha /auth/verify → Validate JWT → Grant/Deny
```

1. Client includes `Authorization: Bearer <token>` header
2. VYUH extracts JWT and calls Suraksha `/auth/verify`
3. Suraksha validates token signature, expiry, and revocation status
4. Response includes user identity and role claims
5. VYUH enforces RBAC based on role claims
...
VYUH retrieves sensitive configuration from Suraksha Vault at startup:
...
| Event | Suraksha Action | VYUH Response |
|-------|----------------|-------------------|

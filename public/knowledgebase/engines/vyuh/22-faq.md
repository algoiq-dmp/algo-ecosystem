# VYUH — Frequently Asked Questions
...
**Q1: What is VYUH?**
Stock analytics engine — trend detection, relative strength scoring, sector analysis, and opportunity scoring for intelligent stock selection.

**Q2: Which server does it run on?**
ALGO IQ 4 at IP address `192.168.190.104`, serving on ports `3021`.

**Q3: Who owns this service?**
The **Analytics** team. Contact via Slack `#vyuh-support` or email `analytics@algoiq.com`.

### Technical

**Q4: How do I access the API?**
Base URL: `http://192.168.190.104:3021/api/v1/`. Authentication requires a Suraksha-issued JWT token in the `Authorization: Bearer` header.
...
**Q6: How do I check if VYUH is healthy?**
```bash
...
v3.0.0. Check `/api/v1/status` for detailed version information including source module versions.
...
```bash
narad deploy vyuh --version <new-version> --env production
```
...
```bash
narad logs vyuh --tail 100 --follow
```
Or directly via container:
```bash
docker logs -f vyuh-core
```
...
curl -X POST http://192.168.190.106:3110/auth/token \
  -d '{"client_id":"vyuh","client_secret":"<secret>"}'
```

**Q16: What's the disaster recovery plan?**
Warm standby in Mumbai DC2. RTO < 15 minutes, RPO < 1 minute. Monthly DR drills verify readiness. Activate via `narad dr-activate vyuh`.

**Q17: How do I get API documentation?**
Swagger UI is available at `http://192.168.190.104:3021/docs` when the service is running.

**Q18: Is there a rate limit?**
Yes. Default: 1,000 requests/minute per client. Internal services get higher limits (10,000/min). Adjust via `VYUH_RATE_LIMIT` config.

**Q19: How does VYUH handle market holidays?**
...
**Q20: Can I contribute to VYUH development?**

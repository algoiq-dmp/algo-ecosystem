# Delta XI — Deployment Guide

**Version:** 3.2.0 | **Owner:** Analytics | **Last Updated:** 2026-07-25

## Deployment Strategy

Delta XI v3.2.0 deploys on **ALGO IQ 4** (`192.168.190.104`) as containerized services managed by Narad deployment orchestrator.
...
```bash
narad deploy delta-xi --env staging --version 3.2.0 --strategy rolling
```

### 3. Smoke Tests
```bash
parikshak smoke-test --target delta-xi --env staging
```

### 4. Production Deployment
```bash
narad deploy delta-xi --env production --version 3.2.0 \
  --strategy blue-green \
...
```bash
narad rollback delta-xi --env production --version 3.2.0
```

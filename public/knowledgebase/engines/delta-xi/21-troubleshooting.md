# Delta XI — Troubleshooting Guide

**Version:** 3.2.0 | **Owner:** Analytics | **Last Updated:** 2026-07-25

## Common Issues & Solutions

This guide covers frequently encountered issues with Delta XI and their resolution steps.
...
**Diagnosis:**
```bash
docker logs delta-xi-core --tail 50
```

**Common causes:**
1. Database unreachable → Verify `DELTA-XIDB_URL` and network
2. MQ unreachable → Check RabbitMQ status on port 5672
3. Narad not running → Verify Narad on port 3100
4. Port conflict → Check no other process on ports 3020

**Resolution:**
```bash
# Check dependencies
narad health-check --target delta-xi --include-deps
...
curl http://192.168.190.104:3020/api/v1/metrics | grep latency
```
...
```bash
# Scale up workers
narad scale delta-xi --replicas 4

# Clear and warm cache
curl -X POST http://192.168.190.104:3020/api/v1/admin/cache/warm
```
...
rabbitmqctl list_queues name messages_ready messages_unacknowledged | grep delta-xi
```

**Resolution:**
```bash
# Restart MQ consumer connection
narad restart-component delta-xi --component mq-consumer
...
**Diagnosis:**
```bash
docker logs delta-xi-core | grep -i migration
```

**Resolution:**
```bash
# Check migration status
docker compose run --rm delta-xi-core ./migrate.sh status

# Repair if stuck
docker compose run --rm delta-xi-core ./migrate.sh repair

# Force rollback and retry
docker compose run --rm delta-xi-core ./migrate.sh undo
docker compose run --rm delta-xi-core ./migrate.sh up
```
...
curl -H "Authorization: Bearer <test_token>" \
  http://192.168.190.104:3020/api/v1/health
```
...
narad reauth delta-xi --service suraksha
docker compose restart delta-xi-api
```
...
narad metrics delta-xi --metric memory_usage --duration 1h
```

**Resolution:**
```bash
# Immediate: restart the service
narad restart delta-xi --graceful
...
curl http://192.168.190.104:3020/api/v1/status | jq .last_processed_at
```

**Resolution:**
```bash
# Check upstream data flow
narad pipeline-status --target delta-xi

# Force reprocessing
curl -X POST http://192.168.190.104:3020/api/v1/admin/reprocess \
...
curl -X POST http://192.168.190.104:3020/api/v1/admin/cleanup \
  -d '{"before":"2026-07-18","force":false}'
```

## Escalation Path

1. Check this guide → 2. Check Grafana dashboards → 3. Slack #delta-xi-support → 4. Page Analytics on-call → 5. Incident in PagerDuty

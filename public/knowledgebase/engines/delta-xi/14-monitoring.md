# Delta XI — Monitoring & Alerting

**Version:** 3.2.0 | **Owner:** Analytics | **Last Updated:** 2026-07-25

## Health Monitoring

Delta XI health is monitored at 99.7% uptime via Narad heartbeat checks every 5 seconds.
...
```prometheus
# Service health
delta-xi_up{instance="192.168.190.104:3020"} 1

# Request metrics
delta-xi_http_requests_total{method="GET", endpoint="/api/v1/health", status="200"}
delta-xi_http_request_duration_seconds{quantile="0.99"}

# Processing metrics
delta-xi_messages_processed_total
delta-xi_processing_duration_seconds{quantile="0.95"}
delta-xi_cache_hit_ratio
```
...
|-----------|-----|---------|
| Service Overview | `/d/delta-xi-overview` | Health, latency, error rates |
| API Insights | `/d/delta-xi-api` | Endpoint usage, rate limiting |
| MQ Consumer | `/d/delta-xi-mq` | Message processing, lag, DLQ |
| Resource Usage | `/d/delta-xi-resources` | CPU, memory, disk, network |

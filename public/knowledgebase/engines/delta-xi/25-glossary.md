...
Key terms used throughout Delta XI documentation and operations.

### A

**API Gateway:** The public-facing REST interface component (`delta-xi-signals`) that handles authentication, rate limiting, and request routing.
...
**Docker Compose:** Tool used for defining and running multi-container Delta XI deployments with a single configuration file.
...
**gRPC:** High-performance RPC framework used for internal inter-module communication within Delta XI.
...
**Webhook:** HTTP callback mechanism where Delta XI sends POST requests to registered subscriber URLs on data change events.

# Delta XI — Suraksha Integration

**Version:** 3.2.0 | **Owner:** Analytics | **Last Updated:** 2026-07-25

## Suraksha Security Integration

Delta XI integrates with **Suraksha** (Security Layer) for all authentication, authorization, and audit capabilities.
...
```
Client → Delta XI API → Suraksha /auth/verify → Validate JWT → Grant/Deny
```

1. Client includes `Authorization: Bearer <token>` header
2. Delta XI extracts JWT and calls Suraksha `/auth/verify`
3. Suraksha validates token signature, expiry, and revocation status
4. Response includes user identity and role claims
5. Delta XI enforces RBAC based on role claims
...
Delta XI retrieves sensitive configuration from Suraksha Vault at startup:
...
| Event | Suraksha Action | Delta XI Response |
|-------|----------------|-------------------|

# Delta XI — Testing Strategy

**Version:** 3.2.0 | **Owner:** Analytics | **Last Updated:** 2026-07-25

## Test Plan

Delta XI follows a comprehensive testing strategy validated through **Parikshak** testing engine.
...
- **Location:** `delta-xi/tests/unit/`
- **Run:** Every commit via CI pipeline
- **Time:** < 2 minutes for full suite

### 2. Component Tests
- **Scope:** Each module (`delta-xi-scanner, delta-xi-signals, delta-xi-api`) tested in isolation
...
Before any release, Delta XI must pass Parikshak certification:

```
parikshak certify delta-xi --version 3.2.0 --env staging
...
test:
  stage: test
  script:
    - parikshak run-unit --target delta-xi
    - parikshak run-component --target delta-xi
    - parikshak security-scan --target delta-xi
  only:
    - merge_requests
    - main
```

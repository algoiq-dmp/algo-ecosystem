# TalkDelta AI — Troubleshooting Guide

**Version:** 1.4.0 | **Owner:** AI/ML | **Last Updated:** 2026-07-25

## Common Issues & Solutions

This guide covers frequently encountered issues with TalkDelta AI and their resolution steps.
...
**Diagnosis:**
```bash
docker logs talkdelta-ai-core --tail 50
```

**Common causes:**
1. Database unreachable → Verify `TALKDELTA-AIDB_URL` and network
2. MQ unreachable → Check RabbitMQ status on port 5672
3. Narad not running → Verify Narad on port 3100
4. Port conflict → Check no other process on ports 3010

**Resolution:**
```bash
# Check dependencies
narad health-check --target talkdelta-ai --include-deps
...
curl http://192.168.190.104:3010/api/v1/metrics | grep latency
```
...
```bash
# Scale up workers
narad scale talkdelta-ai --replicas 4

# Clear and warm cache
curl -X POST http://192.168.190.104:3010/api/v1/admin/cache/warm
```
...
rabbitmqctl list_queues name messages_ready messages_unacknowledged | grep talkdelta-ai
```

**Resolution:**
```bash
# Restart MQ consumer connection
narad restart-component talkdelta-ai --component mq-consumer

# For persistent issues, check network between servers
narad network-test --source 192.168.190.104 --target 192.168.190.118:5672
```
...
**Diagnosis:**
```bash
docker logs talkdelta-ai-core | grep -i migration
```

**Resolution:**
```bash
# Check migration status
docker compose run --rm talkdelta-ai-core ./migrate.sh status

# Repair if stuck
docker compose run --rm talkdelta-ai-core ./migrate.sh repair

# Force rollback and retry
docker compose run --rm talkdelta-ai-core ./migrate.sh undo
...
curl -H "Authorization: Bearer <test_token>" \
  http://192.168.190.104:3010/api/v1/health
```
...
narad reauth talkdelta-ai --service suraksha
docker compose restart talkdelta-ai-api
```
...
narad metrics talkdelta-ai --metric memory_usage --duration 1h
```

**Resolution:**
```bash
# Immediate: restart the service
narad restart talkdelta-ai --graceful

# Long-term: profile with heap dump
# Open ticket with AI/ML team for investigation
```
...
curl http://192.168.190.104:3010/api/v1/status | jq .last_processed_at
```

**Resolution:**
```bash
# Check upstream data flow
narad pipeline-status --target talkdelta-ai

# Force reprocessing
curl -X POST http://192.168.190.104:3010/api/v1/admin/reprocess \
...
curl -X POST http://192.168.190.104:3010/api/v1/admin/cleanup \
  -d '{"before":"2026-07-18","force":false}'
```

## Escalation Path

1. Check this guide → 2. Check Grafana dashboards → 3. Slack #talkdelta-ai-support → 4. Page AI/ML on-call → 5. Incident in PagerDuty

# TalkDelta AI — Frequently Asked Questions
...
ALGO IQ 4 at IP address `192.168.190.104`, serving on ports `3010`.

**Q3: Who owns this service?**
The **AI/ML** team. Contact via Slack `#talkdelta-ai-support` or email `ai-ml@algoiq.com`.

### Technical

**Q4: How do I access the API?**
Base URL: `http://192.168.190.104:3010/api/v1/`. Authentication requires a Suraksha-issued JWT token in the `Authorization: Bearer` header.

**Q5: What databases does it use?**
MongoDB, Redis. The primary database handles transactional data, with Redis for caching and optional TimescaleDB/InfluxDB for time-series data.

**Q6: How do I check if TalkDelta AI is healthy?**
```bash
...
v1.4.0. Check `/api/v1/status` for detailed version information including source module versions.
...
```bash
narad deploy talkdelta-ai --version <new-version> --env production
```
...
```bash
narad logs talkdelta-ai --tail 100 --follow
```
Or directly via container:
```bash
docker logs -f talkdelta-ai-core
```
...
curl -X POST http://192.168.190.106:3110/auth/token \
  -d '{"client_id":"talkdelta-ai","client_secret":"<secret>"}'
```

**Q16: What's the disaster recovery plan?**
Warm standby in Mumbai DC2. RTO < 15 minutes, RPO < 1 minute. Monthly DR drills verify readiness. Activate via `narad dr-activate talkdelta-ai`.

**Q17: How do I get API documentation?**
Swagger UI is available at `http://192.168.190.104:3010/docs` when the service is running.

**Q18: Is there a rate limit?**
Yes. Default: 1,000 requests/minute per client. Internal services get higher limits (10,000/min). Adjust via `TALKDELTA-AI_RATE_LIMIT` config.

**Q19: How does TalkDelta AI handle market holidays?**
...
**Q20: Can I contribute to TalkDelta AI development?**
Contact the AI/ML team. All contributions go through the standard workflow: branch → PR → Parikshak certification → code review → merge to main.

...
```mermaid
graph LR
    A[Ganesh OHLC] -->|REST| B[TalkDelta AI Core]
...
4. **Enrichment:** Cross-referencing with cached historical context from `MongoDB, Redis`
5. **Publication:** Results pushed to MQ topic `talkdelta-ai.output` and exposed via REST

## Output Distribution

- **REST APIs** on ports `3010` serve downstream consumers with pagination support

# TalkDelta — Testing Strategy

**Version:** 5.1.0 | **Owner:** Analytics | **Last Updated:** 2026-07-25

## Test Plan

TalkDelta follows a comprehensive testing strategy validated through **Parikshak** testing engine.
...
- **Location:** `talkdelta/tests/unit/`
- **Run:** Every commit via CI pipeline
- **Time:** < 2 minutes for full suite

### 2. Component Tests
- **Scope:** Each module (`talkdelta-dashboard, talkdelta-analytics, talkdelta-api, talkdelta-stream`) tested in isolation
...
Before any release, TalkDelta must pass Parikshak certification:

```
parikshak certify talkdelta --version 5.1.0 --env staging
...
test:
  stage: test
  script:
    - parikshak run-unit --target talkdelta
    - parikshak run-component --target talkdelta
    - parikshak security-scan --target talkdelta
  only:
    - merge_requests
    - main
```

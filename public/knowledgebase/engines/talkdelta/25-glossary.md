...
Key terms used throughout TalkDelta documentation and operations.

### A

**API Gateway:** The public-facing REST interface component (`talkdelta-analytics`) that handles authentication, rate limiting, and request routing.
...
**Docker Compose:** Tool used for defining and running multi-container TalkDelta deployments with a single configuration file.
...
**gRPC:** High-performance RPC framework used for internal inter-module communication within TalkDelta.
...
**Webhook:** HTTP callback mechanism where TalkDelta sends POST requests to registered subscriber URLs on data change events.

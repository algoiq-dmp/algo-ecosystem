# TalkDelta — Failover & High Availability

**Version:** 5.1.0 | **Owner:** Analytics | **Last Updated:** 2026-07-25

## HA Architecture

TalkDelta maintains 99.8% uptime through redundant deployment and automated failover mechanisms.
...
```
    ...
    └─────┬─────┘            └──────┬─────┘
          │                          │
    ┌─────▼──────────────────────────▼─────┐
    │         Shared PostgreSQL, TimescaleDB, Redis              │
    └──────────────────────────────────────┘
```
...
### Instance Recovery
```bash
narad restart talkdelta --instance 1 --graceful
```
...
# Manual override:
narad failover-db talkdelta --target standby-algo-iq-4
```

### Full Service Recovery
```bash
narad restore talkdelta --from-backup --backup-date latest
```
...
### DR Activation
```bash
narad dr-activate talkdelta --site dr-mumbai
```

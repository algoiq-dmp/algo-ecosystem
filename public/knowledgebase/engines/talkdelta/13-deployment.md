# TalkDelta — Deployment Guide

**Version:** 5.1.0 | **Owner:** Analytics | **Last Updated:** 2026-07-25

## Deployment Strategy

TalkDelta v5.1.0 deploys on **ALGO IQ 4** (`192.168.190.104`) as containerized services managed by Narad deployment orchestrator.
...
```bash
narad deploy talkdelta --env staging --version 5.1.0 --strategy rolling
```

### 3. Smoke Tests
```bash
parikshak smoke-test --target talkdelta --env staging
```

### 4. Production Deployment
```bash
narad deploy talkdelta --env production --version 5.1.0 \
  --strategy blue-green \
...
```bash
narad rollback talkdelta --env production --version 5.1.0
```

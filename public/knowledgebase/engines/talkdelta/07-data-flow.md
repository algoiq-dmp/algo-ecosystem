...
```mermaid
graph LR
    A[Ganesh OHLC] -->|REST| B[TalkDelta Core]
    C[MQ Market Data] -->|Pub/Sub| B
    D[Surya BOD/EOD] -->|REST| B
    E[Lakshmi Live] -->|WebSocket| B
    B --> F[Processing Pipeline]
    F --> G[API Gateway]
    G -->|REST JSON| H[TalkDelta AI, Kavach, Chitragupta, Simulator, Kuber Alpha, Strategy Factory, Delta XI, VYUH, SpreadWatch, Rakshak]
...
4. **Enrichment:** Cross-referencing with cached historical context from `PostgreSQL, TimescaleDB, Redis`
5. **Publication:** Results pushed to MQ topic `talkdelta.output` and exposed via REST

## Output Distribution

- **REST APIs** on ports `3005, 3006` serve downstream consumers with pagination support

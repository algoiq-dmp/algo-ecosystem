# TalkDelta — Frequently Asked Questions
...
**Q1: What is TalkDelta?**
Strategy dashboard and post-trade analytics platform. Publishes APIs for delta calculations, portfolio analytics, strategy synchronization, and risk monitoring.

**Q2: Which server does it run on?**
ALGO IQ 4 at IP address `192.168.190.104`, serving on ports `3005, 3006`.

**Q3: Who owns this service?**
The **Analytics** team. Contact via Slack `#talkdelta-support` or email `analytics@algoiq.com`.

### Technical

**Q4: How do I access the API?**
Base URL: `http://192.168.190.104:3005/api/v1/`. Authentication requires a Suraksha-issued JWT token in the `Authorization: Bearer` header.

**Q5: What databases does it use?**
PostgreSQL, TimescaleDB, Redis. The primary database handles transactional data, with Redis for caching and optional TimescaleDB/InfluxDB for time-series data.

**Q6: How do I check if TalkDelta is healthy?**
```bash
...
v5.1.0. Check `/api/v1/status` for detailed version information including source module versions.

**Q8: How does it communicate with other services?**
Via REST (HTTP/TLS), AMQP (RabbitMQ pub/sub), and Narad TCP connector (service registry). It consumes from Ganesh, MQ, Surya, Lakshmi and publishes to TalkDelta AI, Kavach, Chitragupta, Simulator, Kuber Alpha, Strategy Factory, Delta XI, VYUH, SpreadWatch, Rakshak.

### Operations

**Q9: How do I deploy a new version?**
```bash
narad deploy talkdelta --version <new-version> --env production
```
...
```bash
narad logs talkdelta --tail 100 --follow
```
Or directly via container:
```bash
docker logs -f talkdelta-core
```
...
curl -X POST http://192.168.190.106:3110/auth/token \
  -d '{"client_id":"talkdelta","client_secret":"<secret>"}'
```

**Q16: What's the disaster recovery plan?**
Warm standby in Mumbai DC2. RTO < 15 minutes, RPO < 1 minute. Monthly DR drills verify readiness. Activate via `narad dr-activate talkdelta`.

**Q17: How do I get API documentation?**
Swagger UI is available at `http://192.168.190.104:3005/docs` when the service is running.

**Q18: Is there a rate limit?**
Yes. Default: 1,000 requests/minute per client. Internal services get higher limits (10,000/min). Adjust via `TALKDELTA_RATE_LIMIT` config.

**Q19: How does TalkDelta handle market holidays?**
...
**Q20: Can I contribute to TalkDelta development?**
